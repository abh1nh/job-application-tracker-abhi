...
async function getValidAccessToken(userId) {
  const { data: tokenData, error } = await supabase.from('gmail_tokens').select('*').eq('user_id', userId).single();
  if (error || !tokenData) {
...
async function extractJobInfo(emailContent, subject) {
  if (!openAIApiKey) {
    return {
      isJobRelated: false,
      extractedData: null
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an AI assistant that analyzes emails for job application information. Always respond with valid JSON.'
          },
          {
            role: 'user',
            content: prompt
...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: userData } = await supabase.from('users').select('last_email_scan_at').eq('id', user.id).single();
    ...
    const gmailResponse = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages?q=${encodeURIComponent(query)}&maxResults=50`, {
      headers: {
    ...
    for (const message of messages){
      try {
        ...
        const { data: existingEvent } = await supabase.from('email_events').select('id').eq('message_id', emailData.id).eq('user_id', user.id).maybeSingle();
        ...
        if (isJobRelated && extractedData && extractedData.company && extractedData.position) {
          const { error: jobError } = await supabase.from('job_entries').insert({
            user_id: user.id,
